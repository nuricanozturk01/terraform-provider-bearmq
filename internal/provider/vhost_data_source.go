...
func (d *vhostDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"username":   computedDataString("AMQP username for this virtual host."),
			"domain":     computedDataString("Per-vhost host label. The broker endpoint is TLS-only: `amqps://<username>:<password>@<broker-host>:5671/<name>`."),
			"url":        computedDataString("Connection hint echoed by the broker. Assemble the client URI as `amqps://<username>:<password>@<broker-host>:5671/<name>` (AMQPS, port 5671)."),
