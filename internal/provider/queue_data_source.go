...
func (d *queueDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"durable":           schema.BoolAttribute{Computed: true, MarkdownDescription: "Whether the queue survives a broker restart."},
			"exclusive":         schema.BoolAttribute{Computed: true, MarkdownDescription: "Whether the queue is bound to a single connection."},
			"auto_delete":       schema.BoolAttribute{Computed: true, MarkdownDescription: "Whether the queue is deleted after its last consumer disconnects."},
			...
			"max_message_count": schema.Int64Attribute{Computed: true, MarkdownDescription: "Configured maximum message count."},
		},
	}
}
